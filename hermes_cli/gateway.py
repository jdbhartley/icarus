...
from icarus_branding import (
    COMMAND_NAME,
    GATEWAY_LAUNCHD_LABEL_BASE,
    GATEWAY_SERVICE_DESCRIPTION,
    GATEWAY_SERVICE_NAME_BASE,
)
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "icarus-gateway*", "hermes-gateway*",
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print(f"  This is usually a manual foreground/tmux/nohup run, so `{COMMAND_NAME} gateway`")
...
_SERVICE_BASE = os.getenv("ICARUS_GATEWAY_SERVICE_NAME_BASE", "icarus-gateway")
_LEGACY_SERVICE_BASE = GATEWAY_SERVICE_NAME_BASE
_LAUNCHD_LABEL_BASE = os.getenv("ICARUS_GATEWAY_LAUNCHD_LABEL_BASE", "ai.icarus.gateway")
SERVICE_DESCRIPTION = GATEWAY_SERVICE_DESCRIPTION
...
def get_service_name() -> str:
    ...
    Default installs now use ``icarus-gateway``.
    Profile homes append the profile name (for example
    ``icarus-gateway-coder``). Any other HERMES_HOME appends a short hash for
    uniqueness.
    """
...
# Legacy service names from older Hermes/Icarus installs that predate the
# current Icarus-first service naming. Kept as an explicit allowlist (NOT a
# glob) so profile units and unrelated third-party services are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = tuple(
    dict.fromkeys(
        name
        for name in (
            "hermes.service",
            f"{_LEGACY_SERVICE_BASE}.service",
        )
        if name
    )
)
...
def print_legacy_unit_warning() -> None:
    ...
    print_info(f"    {COMMAND_NAME} gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print(f"Skipped. Run again with: {COMMAND_NAME} gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info(f"  Re-run with: sudo {COMMAND_NAME} gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info(f"    {COMMAND_NAME} gateway uninstall")
    print_info(f"    sudo {COMMAND_NAME} gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            ...
            if run_as_user:
                print_info(f"  After setup, run: sudo {COMMAND_NAME} gateway install --system --run-as-user {run_as_user}")
            else:
                print_info(f"  After setup, run: sudo {COMMAND_NAME} gateway install --system --run-as-user <your-user>")
            print_info(f"  Then start it with: sudo {COMMAND_NAME} gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default installs use ``ai.icarus.gateway.plist`` and profile homes append
    the profile suffix.
    """
    suffix = _profile_suffix()
    name = f"{_LAUNCHD_LABEL_BASE}-{suffix}" if suffix else _LAUNCHD_LABEL_BASE
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    ...
    the calling user's home. This translates it to the target user's equivalent
    path while preserving either the new ``~/.icarus`` default or the legacy
    ``~/.hermes`` layout when explicitly in use:
      /root/.icarus                    → /home/alice/.icarus
      /root/.icarus/profiles/coder     → /home/alice/.icarus/profiles/coder
    ...
    candidate_roots = (
        (current_home / ".icarus", target_home / ".icarus"),
        (current_home / ".hermes", target_home / ".hermes"),
    )

    for current_default, target_default in candidate_roots:
        try:
            relative = current_hermes.relative_to(current_default.resolve())
        except ValueError:
            continue
        return str(target_default / relative) if relative.parts else str(target_default)

    # Completely custom path (not under the invoking user's standard runtime
    # home) — keep as-is.
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Icarus install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}{COMMAND_NAME} gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}{COMMAND_NAME} gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        print(
            ...
            f"  Check status: {'sudo ' if system else ''}{COMMAND_NAME} gateway status\n"
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}{COMMAND_NAME} gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}{COMMAND_NAME} gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}{COMMAND_NAME} gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"{_LAUNCHD_LABEL_BASE}-{suffix}" if suffix else _LAUNCHD_LABEL_BASE
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Icarus install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print(f"  {COMMAND_NAME} gateway status             # Check status")
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Icarus install")
    else:
        print("⚠ Service definition is stale relative to the current Icarus install")
        print(f"  Run: {COMMAND_NAME} gateway start")
