#!/usr/bin/env python3
"""
Icarus CLI - Main entry point.

Usage:
    icarus                     # Interactive chat (default)
    icarus chat                # Interactive chat
    icarus gateway             # Run gateway in foreground
    icarus gateway start       # Start gateway as service
    icarus gateway stop        # Stop gateway service
    icarus gateway status      # Show gateway status
    icarus gateway install     # Install gateway service
    icarus gateway uninstall   # Uninstall gateway service
    icarus setup               # Interactive setup wizard
    icarus logout              # Clear stored authentication
    icarus status              # Show status of all components
    icarus cron                # Manage cron jobs
    icarus cron list           # List cron jobs
    icarus cron status         # Check if cron scheduler is running
    icarus doctor              # Check configuration and dependencies
    icarus honcho setup                    # Configure Honcho AI memory integration
    icarus honcho status                   # Show Honcho config and connection status
    icarus honcho sessions                 # List directory → session name mappings
    icarus honcho map <name>               # Map current directory to a session name
    icarus honcho peer                     # Show peer names and dialectic settings
    icarus honcho peer --user NAME         # Set user peer name
    icarus honcho peer --ai NAME           # Set AI peer name
    icarus honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    icarus honcho mode                     # Show current memory mode
    icarus honcho mode [hybrid|honcho|local]  # Set memory mode
    icarus honcho tokens                   # Show token budget settings
    icarus honcho tokens --context N       # Set session.context() token cap
    icarus honcho tokens --dialectic N     # Set dialectic result char cap
    icarus honcho identity                 # Show AI peer identity representation
    icarus honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    icarus honcho migrate                  # Step-by-step migration guide: OpenClaw native → Icarus + Honcho
    icarus version             Show version
    icarus update              Update to latest version
    icarus uninstall           Uninstall Icarus Agent
    icarus acp                 Run as an ACP server for editor integration
    icarus sessions browse     Interactive session picker with search

    icarus claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    if not sys.stdin.isatty():
        print(
            f"Error: '{command_example(command_name)}' requires an interactive terminal.\n"
...
from icarus_branding import DOCS_URL, PRODUCT_NAME, command_example, display_cli_command
...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'icarus setup' at any time to configure.")
        sys.exit(1)
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: icarus gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  icarus gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Icarus Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  icarus gateway")
        ...
        print()
        print("  Or install as a service: icarus gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'icarus whatsapp' to try again.")
...
def _update_via_zip(args):
    """Update Icarus Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/jdbhartley/icarus/archive/refs/heads/{branch}.zip"
    )
...
OFFICIAL_REPO_URLS = {
    "https://github.com/jdbhartley/icarus.git",
    "git@github.com:NousResearch/hermes-agent.git",
    "https://github.com/jdbhartley/icarus",
    "git@github.com:NousResearch/hermes-agent",
}
OFFICIAL_REPO_URL = "https://github.com/jdbhartley/icarus.git"
    ...
    if not has_upstream:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/jdbhartley/icarus.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/jdbhartley/icarus.git' to add later."
            )
...
def cmd_update(args):
    """Update Icarus Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Icarus Agent")
        return
    ...
    try:
        ...
        try:
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: icarus gateway run")
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print()
                print("    icarus gateway migrate-legacy")
                print()
        ...
        print("  icarus model              # Select provider and model")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=f"""
Examples:
    {cli_name}                        Start interactive chat
    {cli_name} chat -q "Hello"        Single query mode
    {cli_name} -c                     Resume the most recent session
    {cli_name} -c "my project"        Resume a session by name (latest in lineage)
    {cli_name} --resume <session_id>  Resume a specific session by ID
    {cli_name} setup                  Run setup wizard
    {cli_name} logout                 Clear stored authentication
    {cli_name} auth add <provider>    Add a pooled credential
    {cli_name} auth list              List pooled credentials
    {cli_name} auth remove <p> <t>    Remove pooled credential by index, id, or label
    {cli_name} auth reset <provider>  Clear exhaustion status for a provider
    {cli_name} model                  Select default model
    {cli_name} config                 View configuration
    {cli_name} config edit            Edit config in $EDITOR
    {cli_name} config set model gpt-4 Set a config value
    {cli_name} gateway                Run messaging gateway
    {cli_name} -s hermes-agent-dev,github-auth
    {cli_name} -w                     Start in isolated git worktree
    {cli_name} gateway install        Install gateway background service
    {cli_name} sessions list          List past sessions
    {cli_name} sessions browse        Interactive session picker
    {cli_name} sessions rename ID T   Rename/title a session
    {cli_name} logs                   View agent.log (last 50 lines)
    {cli_name} logs -f                Follow agent.log in real time
    {cli_name} logs errors            View errors.log
    {cli_name} logs --since 1h        Lines from the last hour
    {cli_name} debug share             Upload debug report for support
...
""",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Icarus Agent",
    )
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Icarus Agent with an interactive wizard. "
        "Run a specific section: icarus setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Icarus Agent components",
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Icarus Agent setup",
    )
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Icarus Agent. Use 'icarus debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    icarus debug share              Upload debug report and print URL
    icarus debug share --lines 500  Include more log lines
    icarus debug share --expire 30  Keep paste for 30 days
    icarus debug share --local      Print report locally (no upload)
...
""",
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'icarus debug share'",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Icarus Agent configuration",
    )
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Icarus Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Icarus Agent",
        description="Remove Icarus Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Icarus Agent as an ACP (Agent Client Protocol) server",
        description="Start Icarus Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Icarus Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Icarus instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Icarus Agent web dashboard for managing config, API keys, and sessions",
    )
