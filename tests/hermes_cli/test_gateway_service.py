...
class TestSystemUnitHermesHome:
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'HERMES_HOME=/home/alice/.icarus' in unit
        assert '/root/.icarus' not in unit
...
class TestHermesHomeForTargetUser:
    ...
    def test_remaps_default_home(self, monkeypatch):
        ...
        assert result == "/home/alice/.icarus"
...
class TestProfileArg:
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.icarus.gateway-orcha.plist"
...
class TestLegacyHermesUnitDetection:
    ...
    def test_ignores_profile_unit_hermes_gateway_coder(self, tmp_path, monkeypatch):
        ...
        Teknium's concern — legacy profile units such as
        ``hermes-gateway-coder.service`` must not be flagged. The detector is
        an explicit allowlist, not a glob, so profile units are safe.
        """
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        # Drop profile units in BOTH scopes with our ExecStart. Also drop the
        # current default Icarus unit name, which must never be treated as legacy.
        for base in (user_dir, system_dir):
            ...
            (base / "icarus-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "icarus gateway migrate-legacy" in out
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo icarus gateway migrate-legacy" in out
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        default_unit = user_dir / "icarus-gateway.service"
