...
The PID file lives at ``{HERMES_HOME}/gateway.pid``.  New installs default to
``~/.icarus`` (with ``HERMES_HOME``/``ICARUS_HOME`` overrides), which means
...
_GATEWAY_KIND = "icarus-gateway"
_LEGACY_GATEWAY_KINDS = {_GATEWAY_KIND, "hermes-gateway"}
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("ICARUS_GATEWAY_LOCK_DIR") or os.getenv("HERMES_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    icarus_dir = state_home / "icarus" / _LOCKS_DIRNAME
    legacy_dir = state_home / "hermes" / _LOCKS_DIRNAME
    return legacy_dir if legacy_dir.exists() and not icarus_dir.exists() else icarus_dir
