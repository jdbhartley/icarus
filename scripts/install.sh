#!/bin/bash
# ============================================================================
# Icarus Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/jdbhartley/icarus/main/scripts/install.sh | bash
#
# Or with options:
...
# Configuration
REPO_URL_SSH="git@github.com:jdbhartley/icarus.git"
REPO_URL_HTTPS="https://github.com/jdbhartley/icarus.git"
ICARUS_HOME="${ICARUS_HOME:-$HOME/.icarus}"
HERMES_HOME="${HERMES_HOME:-$ICARUS_HOME}"
INSTALL_DIR="${ICARUS_INSTALL_DIR:-${HERMES_INSTALL_DIR:-$HERMES_HOME/icarus-agent}}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --dir)
            ...
            shift 2
            ;;
        --icarus-home|--hermes-home)
            HERMES_HOME="$2"
            shift 2
            ;;
        -h|--help)
            echo "Icarus Agent Installer"
            echo ""
            ...
            echo "  --dir PATH     Installation directory (default: ~/.icarus/icarus-agent)"
            echo "  --icarus-home PATH  Data directory (default: ~/.icarus, or \$ICARUS_HOME / \$HERMES_HOME)"
...
get_icarus_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/icarus" ]; then
        echo "$link_dir/icarus"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/jdbhartley/icarus/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$HERMES_HOME/node/bin/node" ]; then
        ...
        log_success "Node.js $found_ver found (Icarus-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.icarus/node/..."
    ...
    # Place into ~/.icarus/node/ and symlink binaries to ~/.local/bin/
    ...
    log_success "Node.js $installed_ver installed to ~/.icarus/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Icarus Agent itself does not require or retain root access."
...
setup_path() {
    log_info "Setting up icarus command..."
    ...
    # Prefer the new `icarus` command but keep `hermes` as a compatibility alias.
    mkdir -p "$command_link_dir"
    ln -sf "$HERMES_BIN" "$command_link_dir/icarus"
    ln -sf "$HERMES_BIN" "$command_link_dir/hermes"
    log_success "Symlinked icarus → $command_link_display_dir/icarus"
    log_success "Symlinked hermes → $command_link_display_dir/hermes"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "icarus command ready (with hermes alias)"
        return 0
    fi
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Icarus Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Icarus Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so icarus works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.icarus directory structure (config at top level, code in subdir)
    mkdir -p "$HERMES_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.icarus/.env (top level, easy to find)
    if [ ! -f "$HERMES_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$HERMES_HOME/.env"
            log_success "Created ~/.icarus/.env from template"
        else
            touch "$HERMES_HOME/.env"
            log_success "Created ~/.icarus/.env"
        fi
    else
        log_info "~/.icarus/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.icarus/config.yaml (top level, easy to find)
    if [ ! -f "$HERMES_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$HERMES_HOME/config.yaml"
            log_success "Created ~/.icarus/config.yaml from template"
        fi
    else
        log_info "~/.icarus/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$HERMES_HOME/SOUL.md" ]; then
        cat > "$HERMES_HOME/SOUL.md" << 'SOUL_EOF'
# Icarus Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Icarus communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.icarus/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.icarus/"

    # Seed bundled skills into ~/.icarus/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.icarus/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.icarus/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$HERMES_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$HERMES_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.icarus/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Icarus itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (icarus --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'icarus setup' after install."
        return 0
    fi
    ...
    # Run icarus setup using the venv Python directly (no activation needed).
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Icarus to send/receive messages."
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "Running 'icarus whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                HERMES_CMD="$(get_icarus_command_path)"
                $HERMES_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'icarus whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'icarus gateway install' later."
        return 0
    fi
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: icarus gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: icarus gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.icarus/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: icarus gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: icarus gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.icarus/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.icarus/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.icarus/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.icarus/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.icarus/icarus-agent/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}icarus${NC}              Start chatting"
    echo -e "   ${GREEN}icarus setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}icarus config${NC}       View/edit configuration"
    ...
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'icarus' and 'hermes' were linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use the 'icarus' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
