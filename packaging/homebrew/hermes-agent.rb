class HermesAgent < Formula
  ...
  # Formula name and filename stay `hermes-agent` for Homebrew upgrade
  ...
  def install
    ...
    bin.install_symlink bin/"hermes" => "icarus"
  end

  test do
    assert_match "Icarus Agent v#{version}", shell_output("#{bin}/icarus version")

    managed = shell_output("#{bin}/icarus update 2>&1")
