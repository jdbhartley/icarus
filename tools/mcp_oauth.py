...
This module provides the glue:
    - ``IcarusTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Icarus"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses the runtime home dir so each profile gets its own OAuth tokens.
    Layout: ``<runtime-home>/mcp-tokens/``
    """
    try:
        ...
        base = Path(
            ...
            or os.environ.get("ICARUS_HOME")
            or str(Path.home() / ".icarus")
        )
...
# ---------------------------------------------------------------------------
# IcarusTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class IcarusTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        <runtime-home>/mcp-tokens/<server_name>.json         -- tokens
        <runtime-home>/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Icarus records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Icarus.</p></body></html>"
            ) if code else (
...
# Public API
# ---------------------------------------------------------------------------
# Backward-compatible alias for existing imports/tests that still use the
# historical HermesTokenStorage name.
HermesTokenStorage = IcarusTokenStorage


def remove_oauth_tokens(server_name: str) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    storage = IcarusTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Icarus")
    scope = cfg.get("scope")
