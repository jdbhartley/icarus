"""
Icarus MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Icarus-specific extra)

Usage:
    icarus mcp serve
    icarus mcp serve --verbose
...
logger = logging.getLogger("icarus.mcp_serve")
...
def _get_sessions_dir() -> Path:
    ...
    try:
        ...
        home = os.environ.get("HERMES_HOME") or os.environ.get("ICARUS_HOME") or str(Path.home() / ".icarus")
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        home = os.environ.get("HERMES_HOME") or os.environ.get("ICARUS_HOME") or str(Path.home() / ".icarus")
        directory_file = Path(home) / "channel_directory.json"
...
class EventBridge:
    ...
    This is the Icarus equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Icarus MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "icarus",
        instructions=(
            "Icarus messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Icarus MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
